    ...
    this.manager = new Manager({
      ...
      .on("trackStuck", (player, err) => {
        ...
        let errorEmbed = new MessageEmbed()
          ...
          .setDescription(`Ha ocurrido un error al buscar: \`${title}\``)
          .setFooter({
            text: "Tranquilo, algo se rompió pero no es tu culpa.",
          });
        client.channels.cache
          .get(player.textChannel)
          .send({ embeds: [errorEmbed] });
      ...
      .on(
        ...
        /** @param {EpicPlayer} player */ async (player, track) => {
          ...
          let trackStartedEmbed = this.Embed()
            .setAuthor({ name: "Ahora reproduciendo", iconURL: this.config.iconURL })
            .setDescription(
        ...
        /** @param {EpicPlayer} */ async (player, track) => {
          ...
          if (autoQueue) {
            ...
            let queueEmbed = new MessageEmbed()
              .setColor(client.config.embedColor)
              .setAuthor({
                name: "La lista ha finalizado.",
                iconURL: client.config.iconURL,
              })
              .setFooter({ text: "Lista finalizada" })
              .setTimestamp();
            ...
            try {
              if (!player.playing && !twentyFourSeven) {
                setTimeout(async () => {
                  if (!player.playing && player.state !== "DISCONNECTED") {
                    let disconnectedEmbed = new MessageEmbed()
                      ...
                      .setDescription(
                        `El reproductor se ha desconecto por inactividad.`
                      );
                    ...
                    client.warn(
                      `Reproductor: ${player.options.guild} | Sigue sonando`
                    );
                  }
                ...
                client.warn(
                  `Reproductor: ${
                    player.options.guild
                  } | La lista ha finalizado [${colors.blue("24/7 ACTIVADO")}]`
                );
              } else {
                client.warn(
                  `Algo inesperado ha sucedido con el reproductor ${player.options.guild}`
                );
              }
              player.setNowplayingMessage(client, null);
            } catch (err) {
